                ...
                if (username.length() == 0 || email.length() == 0 || password.length() == 0 || confirm.length() == 0) {
                    ...
                    if (password.compareTo(confirm) == 0) {
                        if (isValid(password)) {
                            ...
                            Toast.makeText(getApplicationContext(), "Password must contain 6 characters with at least one letter, one number and one special character", Toast.LENGTH_SHORT).show();
                        }
                    } else {
